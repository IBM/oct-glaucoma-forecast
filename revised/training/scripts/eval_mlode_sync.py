...
def evaluate_struct_function(model, ts_list_val, inputs, masks, nv_fc_rnfl=3):
    ...
    #print('x.shape[1]=', inputs[1].shape[1])
    #print('nv_fc + 1 = ', nvs[1] + 1)
    #print('x.shape[1] - (nv_fc +1) = ', inputs[1].shape[1] - (nvs[1] + 1))

    #inputs_ = [slice_inputs_target(i, nv) if i is not None and nv > 0 else None for i, nv in zip(inputs, nvs)]
    inputs_ = [slice_inputs_target(i, nv) if i is not None and nv >= 0 else None for i, nv in zip(inputs, nvs)]
    #print('len(inputs_)=', len(inputs_))
    #print('len(inputs_[0])=', len(inputs_[0]))
    #print('len(inputs_[0][0]=', len(inputs_[0][0]))
    ...
    for nv_fc_rnfl in (range(N_t)):  # nv_fc_rnvl [0,1,2,3] we.g, when 3 it uses last 3 visits    
        ...
        for nv_fc_vft in range(N_t):        
            if (nv_fc_rnfl == 0 and nv_fc_vft == 0):
                ...
                res = evaluate_forecast_matrix_base_largergap_hhyu(model, ts_list_val, inputs, masks, modalities, nv_fc_rnfl, nv_fc_vft)
                row.append(res)
...
# HHYU: I think Suman made a mistake in indexing
def evaluate_forecast_matrix_base_largergap_hhyu(model, ts_list_val, inputs, masks, modalities, nv_fc_rnfl, nv_fc_vft):
    ...
    nvs = [nv_fc_rnfl, nv_fc_vft]
    print("nvs=", nvs)
    slice_inputs = lambda x, nv_fc: x[:, list(range(x.shape[1] - (nv_fc + 1), x.shape[1] - 1))]
    inputs_ = [slice_inputs(i, nv) if i is not None and nv > 0 else None for i, nv in zip(inputs, nvs)]
    masks = [slice_inputs(i, nv) if i is not None and nv > 0 else None for i, nv in zip(masks, nvs)]

    slice_inputs_target = lambda x, nv_fc: x[:, list(range(x.shape[1] - (nv_fc + 1), x.shape[1] - 1)) + [-1]]  # inputs + target forecast
    ts_list = [slice_inputs_target(ts, nv) if nv > 0 else None for ts, nv in zip(ts_list_val, nvs)]
...
def evaluate_reconstruction_error(config, data, mode='rec', nv_fc=-1, pretrain_mode=False, start_index=None, target_index=-1):
    ...
    print("HHYU comb=", comb)

    with torch.no_grad():
        ds_val_batches, ts_list_val, val_dx = data.get_val(dx_filter=None)
        #print("HHYU ds_val_batches=", len(ds_val_batches))
        #print("HHYU ts_list_val=", len(ts_list_val))
        #print("HHYU val_dx=", len(val_dx))
        ...
        def evaluate_rec_error(inputs, modalities):
            ...
            # HHYU: Suman's code below caused problems because the first argument needs to be a list of two items
            # based on train_multimodal_latentodegru_sync_pretrain(), the test() function, I decided to duplicate it
            # I don't fully understand the logic
            # ts_list_val_ = subsample(ts_list_val, modalities)  # Suman
            ts_list_val_ = subsample([ts_list_val, ts_list_val], modalities)  # HHYU
        ...
        # HHYU: c goes through [[1, 1], [0, 1], [1, 0]]
        for c in comb:
