...
# HHYU: turn off GPU when testing
...
def compute_forecast_error(model, ts_list, inputs, modality_flags):
    ...
    if(modality_flags[0] ==1):
        # HHYU: took away disc_dia_mm
        rnfl_mask = create_rnfl_mask(inputs[0])
        ...
        # HHYU: the original code multiply by 41. I don't understand why. Changed it to 40
        error = mae_loss(reshape(gt[:, [-1]]) * 40, reshape(pred[:, [-1]]) * 40,
