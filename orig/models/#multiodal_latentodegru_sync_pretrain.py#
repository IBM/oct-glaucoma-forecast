...
class RecognitionODEGRU(nn.Module):
    ...
    def __init__(self, type, latent_dim, sync_function, z_size=128, device=None, ode_solver=None, ):
...
class MultimodalLatentODESync(nn.Module):
    def __init__(self, latent_dim, device=None, ode_solver=None, expert='poe', split_latent=False):
        ...
        #self.h2musigma_rnfl = nn.Linear(nhidden_size_rnfl, latent_dim * 2)
        #self.h2musigma_vft = nn.Linear(nhidden_size_vft, latent_dim * 2)

        self.rnn_rnfl = RecognitionODEGRU(type='rnfl', latent_dim=latent_dim,
                                          device=device,
                                          ode_solver=ode_solver, z_size=nhidden_size_rnfl, sync_function=None)#self.hsyncfunc)


        self.rnn_vft = RecognitionODEGRU(type='vft', latent_dim=latent_dim,
                                         device=device,
                                         ode_solver=ode_solver, z_size=nhidden_size_vft, sync_function=None)#elf.hsyncfunc)
