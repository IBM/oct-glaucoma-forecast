...
class MultimodalTimeSeriesData(VAEData):
    ...
    def __init__(self):
        ...
        train, val, test = get_data_ts_onh_mac(mask_onhrnfl_disc=True)
    ...
    def get_train_minibatch(self, batch_size):
        ...
        N_t=self.train_rnflonh.shape[1]
        tidx =torch.randint(0,N_t,(1,))[0]

        idx = torch.randperm(self.train_rnflonh.shape[0], device=device)

        aug_rnfl_batch = self.train_rnflonh[idx[:batch_size ],tidx:tidx+1] #self.augment_rot(self.train_rnflonh[idx[:batch_size]])

        maps = aug_rnfl_batch, self.train_rnflmac[idx[:batch_size], tidx:tidx+1], self.train_vft[
            idx[:batch_size], tidx:tidx+1]
        ts = self.age_at_vd_train[idx[:batch_size], tidx:tidx+1]
        return maps, ts
    ...
    def get_val(self, dx_filter=None):
        ...
        reshape = lambda x: x.reshape(x.shape[0] * x.shape[1], 1, x.shape[2], x.shape[3], x.shape[4])
        maps = [reshape(m) for m in maps]
        reshape_ = lambda x: x.reshape(x.shape[0] * x.shape[1], 1)
        ts = reshape_(ts)




        dx = torch.max(reshape_(self.val_dx), dim=1)[0]  # (N,1)
        if (dx_filter):
...
def getConfig(modalities_, expert_, latent_dim_):
    class MyConfig(Config):
        ...
        image_dim = 64
        vfim_dim = 32
        LOG_ROOT_DIR = 'debug_temp_mlode' + str(latent_dim)
    ...
    for expert in experts:
        ...
        for mm in modalities_exp:
            ...
            for epoch in range(1, nepochs + 1):
                ...
                rec_eval = get_rec_errors(config, data, mode='rec', nv_fc=3)
