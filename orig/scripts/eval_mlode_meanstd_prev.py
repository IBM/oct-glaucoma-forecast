...
def evaluate_forecast_error(model, ts, inputs, masks, modalities, nv_fc):
    ...
    slice= lambda x: x[:, x.shape[1] - (nv_fc +1):-1]
    #inputs_ = [i[:, :nv_fc] if i is not None else None for i in inputs]
    inputs_ = [slice(i) if i is not None else None for i in inputs]


    inputs_ = subsample(inputs_, modalities)
    outlist, mulogvar = model(ts=ts, x_list=inputs_)
    ...
    rnfl_xi_val, gcl_xi_val, vft_xi_val = inputs
    rnfl_mask, gcl_mask, vft_mask = masks
    error0, std0 = meanstd(mae_globalmean(reshape(rnfl_xi_val[:, [-1]]) * 200,
                       reshape(pred_rnfl[:, [-1]]) * 200, mask=rnfl_mask[:, [-1]])) if pred_rnfl is not None else (NoneError, NoneError)
    error1, std1 = (NoneError, NoneError)

    error2, std2 = meanstd(mae_loss(reshape(vft_xi_val[:, [-1]]) * 40, reshape(pred_vft[:, [-1]]) * 40,
                                 mask=reshape(vft_mask[:, [-1]]))) if pred_vft is not None else (NoneError, NoneError)

    return [error0, error1, error2], [std0, std1, std2], [pred_rnfl, pred_gcl, pred_vft]
...
def evaluate_reconstruction_error(config, data, mode='rec', nv_fc=-1):
    ...
    with torch.no_grad():
        ...
        def evaluate_rec_error(inputs, modalities):
            ...
            reshape = lambda x: x.reshape((x.shape[0] * x.shape[1], x.shape[2], x.shape[3], x.shape[4]))
            #mae_globalmean
            error0, std0 = meanstd(mae_loss(reshape(rnfl_xi_val[:, :]) * 200,
                               reshape(pred_rnfl[:, :]) * 200)) if pred_rnfl is not None else (NoneError, NoneError)
            error1, std1 = meanstd(mae_loss(gcl_xi_val * 200, pred_gcl * 200)) if pred_gcl is not None else (NoneError, NoneError)

            error2, std2= meanstd(mae_loss(reshape(vft_xi_val[:, :]) * 40, reshape(pred_vft[:, :]) * 40,
                                         mask=reshape(vft_mask[:, :]))) if pred_vft is not None else (NoneError, NoneError)
        ...
        for c in comb:
            ...
            if(mode=='rec'):
                error,std, preds = evaluate_rec_error(x_list, c)
            else:
                error, std, preds = evaluate_forecast_error(model,ts_val,x_list,masks,c,nv_fc)

            error = [e.cpu().numpy() for e in error]
            error = [float(e) for e in error]
            error = subsample(error, config.MODALITIES)

            std = [st.cpu().numpy() for st in std]
            std = [float(st) for st in std]
            std = subsample(std, config.MODALITIES)
            ...
            errors.append(error)
            stds.append(std)
            inputs_all.append(x_list)

    return errors, stds, inputs_modalities, preds_all, inputs_all
