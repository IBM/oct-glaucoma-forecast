...
from models.multiodal_latentodegru_sync_pretrain import MultimodalLatentODESync
...
def subsampled_elbo_losses(model, t_ts, inputs, modality_flags, annealing_factor=1.,
    ...
    for c in comb:
        ...
        #this output corresponds to the lowest time point among both modalities but here for pretrained exp
        #we use same ts for both modalities, So next step is selecting first element from inputs_
...
class MultimodalTimeSeriesData(VAEData):
    ...
    def __init__(self, fold_seed):
        ...
        #do not filter in pretrain phase
        #self.filter()
...
def getConfig(modalities_, expert_, latent_dim_, fold_seed_):
    class MyConfig(Config):
        ...
        LOG_ROOT_DIR = 'pretrain_sync_nonadj_euler_ce_temp_mlode' + str(latent_dim) + _suffix
        MODALITIES = modalities_  # [0, 0, 1]
        expert = expert_  # 'moe'
        ode_method = 'euler'

        modalities_str = ''.join([str(xi) for xi in MODALITIES])
        prefix = 'pretrain_sync_multimoda_latentode' + str(latent_dim) + modalities_str + '_' + expert
        ...
        def initialize_model_optimizer(self):
            ...
            print('loading pretrained mvae encoders and decoders')
            #copy rnfl encoder and decoder from mvae
            modalities_adapt = self.MODALITIES.copy()
            modalities_adapt.insert(1,0) # the mvae has format of [ rnfl, gcl, vft] so add 0 for gcl

            ConfigMVAE = getConfigMVAE(modalities_adapt, self.expert, self.latent_dim)
...
def visualize_embedding(config: Config, data: VAEData, epoch=None, type='umap'):
    ...
    with torch.no_grad():
        ...
        ts_val_sub=subsample([ts_val, ts_val], config.MODALITIES)
...
def get_rec_errors(config, data, mode, nv_fc):
    # recontructon error
    out = ''
    errors, inputs_c, preds, _ = evaluate_reconstruction_error(config, data, mode=mode, nv_fc=nv_fc, pretrain_mode=True)
