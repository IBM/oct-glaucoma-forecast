...
# HHYU: I set the default rnfl_dia_mm to 7
def compute_rnfl_error(inputs, preds, rnfl_dia_mm=7, disc_dia_mm=1.92, use_ma=False, quadrant=None, mode='forecast',
                       comb_index=0):
    """
    ...
    # global_mean
    errors_gm = np.abs(
        np.mean(rnfl_masked.reshape(rnfl_masked.shape[0], -1), axis=1) - 
        np.mean(rnfl_pred_masked.reshape(rnfl.shape[0],   -1), axis=1)) * 200
    err_gm, std_gm = np.mean(errors_gm), np.std(errors_gm)
...
if (__name__ == '__main__'):
    ...
    for ei, expert in enumerate(experts):
        ...
        for mi, mm in enumerate(modalities_exp):
            ...
            errors_rnfl_all = []
            errors_rnfl_all_seg = [[], [], [], []]
            errors_rnfl_all_unimodal = []
            errors_rnfl_all_unimodal_seg = [[], [], [], []]
            errors_vft_all = []
            errors_vft_all_unimodal = []
            ...
            #for nv_fc, si, ti in zip([3, 3, 3], [0, 0, 0], [3, 4, 5]):
                ...
                print('# NV ', nv_fc)
                # HHYU: errors are calculated by evaluate_forecast_error(). Upscaled.
                #       errors[0] - global mean
                #       errors[1] - mae
                errors, inputs_c, preds, inputs = evaluate_reconstruction_error(config, 
                                                                                data, 
                                                                                mode = 'forecast',
                                                                                nv_fc = nv_fc, 
                                                                                start_index = si,
                ...
                #print("HHYU:", np.asanyarray(errors).shape)
                ...
                # HHYU: I think this means that vft error is not global mean
                ...
                for ii, e in zip(inputs_c, errors):
                    if ii==[1, 1]:
                        errors_vft_all.append(e[1])
                    if ii==[0, 1]:
                        errors_vft_all_unimodal.append(e[1])
                ...
                for rnfl_comb_index in [0, 2]:
                    ...
                    if (mm[0] == 1): #HHYU: if input includes RNFL
                        # [err, std], [err_gm, std_gm] = compute_rnfl_error(inputs, preds, rnfl_dia_mm=6, use_ma=True)
                        [err, std], [err_gm, std_gm], [abs_err, abs_err_gm] = compute_rnfl_error(inputs, 
                                                                                                 preds,
                                                                                                 rnfl_dia_mm = 7, 
                                                                                                 use_ma = True,
                                                                                                 quadrant = None,
                                                                                                 comb_index = rnfl_comb_index)
                        # HHYU: note that the errors are global means
                        ...
                        for q in [0, 1, 2, 3]:
                            ...
                            if rnfl_comb_index == 0:
                                errors_rnfl_all_seg[q].append(abs_err_gm)
                            else:
                                errors_rnfl_all_unimodal_seg[q].append(abs_err_gm)

            # HHYU: for rnfl, error is calculated by compute_rnfl_errors()
            print('saving rnfl errors with means', [np.mean(e) for e in errors_rnfl_all], 'columns', nvisits_all)

            # HHYu: for vft, error is clalculated by evaluate_reconstruction_error()
            ...
            for q in [0, 1, 2, 3]:
                save_error(model_name='mlode_joint_seg' + str(q), target_modality='rnfl', errors=errors_rnfl_all_seg[q], nv=nvisits_all,
                       exp_type=exp_type, save_dir='results_' + str(fold_seed))               
            
            save_error(model_name='mlode_joint_unimodal', target_modality='rnfl', errors=errors_rnfl_all, nv=nvisits_all,
                       exp_type=exp_type, save_dir='results_' + str(fold_seed))

            for q in [0, 1, 2, 3]:

                save_error(model_name='mlode_joint_unimodal_seg' + str(q), target_modality='rnfl', errors=errors_rnfl_all_unimodal_seg[q], nv=nvisits_all,
                       exp_type=exp_type, save_dir='results_' + str(fold_seed))               
            
            save_error(model_name='mlode_joint', target_modality='vft', errors=errors_vft_all, nv=nvisits_all,
                       exp_type=exp_type, save_dir='results_' + str(fold_seed))
            save_error(model_name='mlode_joint_unimodal', target_modality='vft', errors=errors_vft_all_unimodal, nv=nvisits_all,
                       exp_type=exp_type, save_dir='results_' + str(fold_seed))            
            ...
            print('HHYU key=', key)
            print('HHYU e.shape=', e.shape)

            # HHYU: based on rnfl and vft, forecast rnfl
            e_rnfl = list(np.clip(e[:, 0, 0], 0, 20))

            # HHYU: based on rnfl and vft, forecast vft
            e_vft = list(np.clip(e[:, 0, 1], 0, 15))  # [:,0,2] using rnfl+vft and [:,2,2] only using vft

            # HHYU: based on vft, forecast rnfl
            e_rnfl_fromvft = list(np.clip(e[:, 2, 0], 0, 30))

            # HHYu: based on rnfl, forecast vft
            ...
            # HHYU: I disabled this section, because errors_quads are not calculated (empty list)
            # quadrants
