...
def getConfig(modalities_, expert_, latent_dim_):
    class MyConfig(Config):
        ...
        image_dim = 64
        vfim_dim = 32
        LOG_ROOT_DIR = 'tempmvae_latent_exp_poe'
        ...
        def create_model(self, load_weights=False):
            ...
            if (load_weights):
                ...
                print('Loading pretrained weights ', model_file)
                # latentode.load_state_dict(torch.load(model_file, map_location='cpu'))
                latentode.load_state_dict(torch.load(model_file, map_location=device)) #map_location=torch.device('cpu')
...
if (__name__ == '__main__'):
    ...
    for latent_dim in latent_dims:
        for expert in experts:
            ...
            if (expert == 'moe'):  modalities_exp = [[1, 0, 1]]
