...
from models.multiodal_latentodegru_sync import MultimodalLatentODESync
from scripts.eval_mlode_sync import evaluate_reconstruction_error
...
def plot_zembedded(mu_in, val_dx, save_path, type='tsne'):
    assert type in ['tsne', 'umap'], 'type can be one of umap or tsne'
    labels = {2: 'Glaucoma', 1: 'GS', 0: 'Normal'}

    if (type == 'tsne'):
        mu = TSNE(n_components=2, perplexity=30).fit_transform(mu_in)
    else:
        mu = umap.UMAP(n_neighbors=10,
                       min_dist=0.1, n_components=2,
                       metric='euclidean').fit_transform(mu_in)
...
def subsampled_elbo_losses(model, ts_list, inputs, modality_flags, annealing_factor=1.,
                           batch_size=None):
    """
    ...
    for c, w in zip(comb, weights):
        ...
        # ts_list=[t_ts, t_ts]
        ts_list_sub = subsample(ts_list, c)

        pred_list, [mu, logvar] = model(ts_list_sub, input_xi_sub)
        ...
        loss_, KLD_ = elbo_general_timeseries(preds=pred_list_, gts=inputs_, mu=mu, logvar=logvar,
                                              annealing_factor=annealing_factor, loss_type='se')
...
def process_maps(train, val, test):
    ...
    N, t, H, W, c = test.shape
    test = test.transpose([0, 1, 4, 2, 3])  # (N,t,c,H,W)
...
class MultimodalTimeSeriesData(VAEData):
    ...
    def __init__(self, fold_seed, filter_pr=True, suffix='', idx_r=None):
        self.idx_r = idx_r
        train, val, test = get_data_ts_onh_mac(mask_onhrnfl_disc=True, fold_seed=fold_seed, suffix=suffix)
        self.train_ = train
        self.val_ = val
        self.test_ = test
        ...
        self.train_sid, self.val_sid = process_labels(train[2][1], val[2][1], test[2][1])

        # to years
        self.age_at_vd_train = self.age_at_vd_train / 12.0
        self.age_at_vd_val = self.age_at_vd_val / 12.0

        if (filter_pr): self.filter()
    ...
    def filter(self):
        ...
        idx_trn, mb_ = filteridx_by_progression_rate(self.train_rnflonh * 200, self.age_at_vd_train,
                                                     slope_threshold=0.5)
        idx_trn = idx_trn[0]
        self.train_rnflonh, self.train_rnflmac, self.train_vft, self.age_at_vd_train, self.train_dx, self.train_sid = [
            d[idx_trn] for d
            in
            [
                self.train_rnflonh,
        ...
        self.val_rnflonh, self.val_rnflmac, self.val_vft, self.age_at_vd_val, self.val_dx, self.val_sid = [d[idx_val]
                                                                                                           for d in
                                                                                                           [
                                                                                                               self.val_rnflonh,
                                                                                                               self.val_rnflmac,
                                                                                                               ...
                                                                                                               self.val_dx,
                                                                                                               self.val_sid]]
        if (self.idx_r is not None):
            idx_org = range(self.val_rnflonh.shape[0])
            idx_org_filt = [i for i in idx_org if i not in self.idx_r]
            self.val_rnflonh, self.val_rnflmac, self.val_vft, self.age_at_vd_val, self.val_dx, self.val_sid = [
                d[idx_org_filt] for d
                in
                [self.val_rnflonh,
                 self.val_rnflmac,
                 self.val_vft,
    ...
    def get_train_minibatch(self, batch_size):
        ...
        maps = [aug_rnfl_batch, self.train_vft[idx[:batch_size]]]
        ts = self.age_at_vd_train[idx[:batch_size]]
        ts_list = [ts, ts.clone().detach()]

        if (torch.rand((1,)) > 0.6):
            selid = torch.randperm(2)[0]  # which modality to reduce size
            seltime = torch.randperm(2)[0] + 1  # how much time to remove 1, or 2
            maps[selid] = maps[selid][:, seltime:]
            ts_list[selid] = ts_list[selid][:, seltime:]
    ...
    def get_val_subject_ids(self):
        sids = np.max(self.val_sid.detach().cpu().numpy(), axis=1)
        return sids

    def get_train_subject_ids(self):
        sids = np.max(self.train_sid.detach().cpu().numpy(), axis=1)
        return sids
    ...
    def print_stat(self):
        trn_sids= self.get_train_subject_ids()
        val_sids = self.get_val_subject_ids()
        print('#Train subjects', len(np.unique(trn_sids)), 'observation',  len(trn_sids))
        print('#Val subjects', len(np.unique(val_sids)), 'observation',  len(val_sids))
        #division

        dx_val = torch.max(self.val_dx, dim=1)[0]  # (N,1)
        dx_trn = torch.max(self.train_dx, dim=1)[0]  # (N,1)

        print('#Train Normal Subjects', len(np.unique(trn_sids[dx_trn==0])))
        print('#Train Suspects Subjects', len(np.unique(trn_sids[dx_trn==1])))
        print('#Train Glaucoma Sbjects', len(np.unique(trn_sids[dx_trn == 2])))

        print('#Val Normal Subjects', len(np.unique(val_sids[dx_val==0])))
        print('#Val Suspects Subjects', len(np.unique(val_sids[dx_val==1])))
        print('#Val Glaucoma Subjects', len(np.unique(val_sids[dx_val == 2])))


        print('#Train Normal observation', len((trn_sids[dx_trn == 0])))
        print('#Train Suspects observation', len((trn_sids[dx_trn == 1])))
        print('#Train Glaucoma observation', len((trn_sids[dx_trn == 2])))

        print('#Val Normal observation', len((val_sids[dx_val == 0])))
        print('#Val Suspects observation', len((val_sids[dx_val == 1])))
        print('#Val Glaucoma observation', len((val_sids[dx_val == 2])))
...
from train_multimodal_latentodegru_pretrain import getConfig as getConfigMLODE_pretrained
...
def getConfig(modalities_, expert_, latent_dim_, fold_seed_):
    class MyConfig(Config):
        ...
        #CleanMSEsync - pt sd is 2 : MSEsync - pt sd is 4
        LOG_ROOT_DIR = 'MSEsync_nonadj_euler_ce_temp_mlode' + str(latent_dim) + _suffix
        MODALITIES = modalities_  # [0, 0, 1]
        expert = expert_  # 'moe'
        ode_method = 'euler'

        modalities_str = ''.join([str(xi) for xi in MODALITIES])
        prefix = 'sync_multimoda_latentode' + str(latent_dim) + modalities_str + '_' + expert
        ...
        def initialize_model_optimizer(self, load_pretrained_weights = True):
            """
            ...
            if (load_pretrained_weights):
                modalities_adapt = self.MODALITIES.copy()
                modalities_adapt.insert(1, 0)  # the pretrain config has format of [ rnfl, gcl, vft] so add 0 for gcl
                
                print("HHYU: before loading pretrained model")
                ConfigMLODEPT = getConfigMLODE_pretrained(modalities_adapt, self.expert, self.latent_dim, fold_seed_=self.fold_seed)
        ...
        def create_model(self, load_weights=False, suffix_model_path=''):
            ...
            latentode = MultimodalLatentODESync(latent_dim=config.latent_dim,
...
def train(epoch, config: Config, data: VAEData, niter_per_epoch=None):
    ...
    for itr in range(1, niter_per_epoch + 1):
        ...
        train_elbo_loss, kld = subsampled_elbo_losses(model, ts_list, minibatches, config.MODALITIES,
...
def visualize_embedding(config: Config, data: VAEData, epoch=None, type='umap'):
    ...
    with torch.no_grad():
        ...
        ts_val_sub = subsample(ts_list_val, config.MODALITIES)
        val_dx = val_dx.cpu().numpy()
        mu, _ = model.infer(ts_val_sub, ds_val_batches_sub)
        mu = mu.cpu().numpy()

        if (config.latent_dim == 2):  proj = plot_z(mu, val_dx, save_path=os.path.join(config.RESULT_DIR,
                                                                               'zplots' + str(epoch) + '.jpeg'))
        if (config.latent_dim > 2): proj = plot_zembedded(mu, val_dx, save_path=os.path.join(config.RESULT_DIR,
                                                                                      'embed_zplots' + str(
                                                                                          epoch) + '.jpeg'), type=type)
        return proj, mu, val_dx
...
def test(epoch, config: Config, data: VAEData):
    ...
    with torch.no_grad():
        ...
        val_elbo_loss, kld = subsampled_elbo_losses(model, ts_list_val, ds_val_batches, config.MODALITIES)
...
def viz_latent_space(model, modalities, save_path, image_size):
    ...
    generators = [model.decoder_rnfl, model.decoder_vft]
...
if (__name__ == '__main__'):
    ...
    #Note: Clean is when pre-t models from sed2, otherwuse ptm from sed4
    experts = ['poe']  # ,'moe']#, 'moe']
    ...
    # for seed 2
    #idx_r = [181, 249, 107, 342, 191, 325, 20, 330, 155, 329, 340, 85, 324, 162, 3]  # filter
    idx_r = []

    #latent_dims=[2, 4,8, 16, 32, 64]

    # HHYU: use only latent_dim = 32
    latent_dims = [32]
    fold_seed = 5
    ...
    for latent_dim in latent_dims:
        for expert in experts:
            if (expert == 'moe'): modalities_exp = [[1, 0], [0, 1]] #[1, 1]
            if (expert == 'poe'):  modalities_exp = [[1, 1]]
            ...
            for mm in modalities_exp:
                ...
                config = Config()
                config.initialize_model_optimizer(load_pretrained_weights=load_pretrained_weights)

                # HHYU: add multiple training files
                #for suffix in ["", "5", "6", "7", "8"]:
                for suffix in [""]:                    

                    data = MultimodalTimeSeriesData(fold_seed=config.fold_seed, idx_r=idx_r, suffix=suffix)

                    niter_per_epoch = int(data.train_rnflonh.shape[0] / config.BATCH_SIZE)
                    nepochs = config.EPOCHS

                    print('Training data size', data.size_train(), 'train subjects',
                          np.unique(data.get_train_subject_ids()).shape[0])
                    print('Val data size', data.size_val(), 'val subjects', np.unique(data.get_val_subject_ids()).shape[0])
                    ...
                    # HHYU: I believe this resets the learning rate
                    ...
                    for epoch in range(1, nepochs + 1):
                        ...
                        rec_eval = get_rec_errors(config, data, mode='forecast', nv_fc=3)  # nv_fc=-1 for rec
                        print_losses(config, start, end, extra_str=rec_eval)
                        metrics.append(rec_eval)
                        # np.savetxt
